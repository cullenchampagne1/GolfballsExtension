(function installUsageTelemetry(root) {
  'use strict';

  /**
   * The installation's privacy-bounded utilization reporter.
   *
   * Content scripts send fixed feature/surface events to the MV3 worker. The
   * worker coalesces high-frequency feature actions while preserving each email
   * send as its own row, retains the pending minute in chrome.storage.session
   * across worker eviction, and sends one periodic batch through the
   * installation credential. No typed content, CRM identity, URL, subject,
   * recipient, filename, or search query can enter this contract. Email rows
   * may carry a deterministic template subject-cluster ID, account territory,
   * and the pre-send prior-email timestamp.
   */

  const ADDRESS = `/${['projects', 'golfballs-extension', 'client', 'telemetry'].join('/')}`;
  const ALARM = ['gb', 'usage', 'flush'].join('-');
  const STATE_KEY = 'gbUsageTelemetryStateV2';
  const STATE_VERSION = 3;
  const PERIOD_MINUTES = 1;
  const SOON_MS = 1_500;
  const CAPACITY = 960;
  const FEATURE_CAPACITY = 96;
  const SURFACE_KINDS = new Set(['modal', 'page', 'popup']);
  const FEATURE_KINDS = new Set([
    'email_send',
    'email_preview',
    'call_log',
    'contact_import',
    'contact_import_run',
    'proof_submit',
    'gift_catalog_open',
    'gift_catalog_search',
    'gift_catalog_add',
    'gift_catalog_proposal_save',
    'gift_catalog_publish',
    'gift_catalog_email',
    'gift_catalog_checkout',
  ]);
  const FEATURE_SOURCES = new Set([
    'popup', 'task_list', 'crm_search', 'email_preview', 'contact',
    'submit_proof', 'gift_catalog', 'other',
  ]);
  const TRANSPORTS = new Set(['pa', 'mailto', 'none']);
  const EVENT_KINDS = new Set(['surface_open', 'surface_close', 'latency', 'feature']);
  const SURFACE_MAX = 64;
  const MS_MAX = 15 * 60 * 1000;
  // A backend round-trip is never anywhere near MS_MAX (that ceiling exists for
  // surface_close dwell time, which legitimately runs many minutes) — a
  // `latency` sample this large used to come from a since-fixed bug that timed
  // the notifications long-poll (server-held up to 25s, see
  // lib/notifications-poll.js's LONG_POLL_SECONDS) like an ordinary call. Its
  // own, much tighter cap — safely below that 25s hold — keeps that class of
  // outlier from being recordable again.
  const LATENCY_MS_MAX = 5_000;
  const COUNT_MAX = 1_000_000;

  const compactLabel = (value, maximum) => String(value || '')
    .replace(/[\u0000-\u001f\u007f]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, maximum);

  const finite = (value, max, minimum = 0) => {
    const number = Math.round(Number(value));
    return Number.isFinite(number) && number >= minimum ? Math.min(number, max) : null;
  };

  function normalize(event) {
    if (!event || typeof event !== 'object') return null;
    const kind = String(event.kind || '');
    if (!EVENT_KINDS.has(kind)) return null;
    const out = { kind };
    if (kind === 'feature') {
      const feature = String(event.feature || '');
      const source = String(event.source || 'other');
      const transport = event.transport == null ? null : String(event.transport);
      const count = finite(event.count == null ? 1 : event.count, 10_000, 1);
      if (!FEATURE_KINDS.has(feature) || !FEATURE_SOURCES.has(source) || count === null) return null;
      if (transport !== null && !TRANSPORTS.has(transport)) return null;
      out.feature = feature;
      out.source = source;
      if (transport) out.transport = transport;
      out.count = count;
      out.word_count = finite(event.word_count || 0, COUNT_MAX) || 0;
      out.attachment_count = Math.min(count, finite(event.attachment_count || 0, COUNT_MAX) || 0);
      out.inline_image_count = Math.min(count, finite(event.inline_image_count || 0, COUNT_MAX) || 0);
      if (feature === 'email_send') {
        const subjectClusterId = compactLabel(event.subject_cluster_id, 8192);
        const templateId = compactLabel(event.template_id, 200);
        const templateName = compactLabel(event.template_name, 160);
        const variationId = compactLabel(event.template_variation_id, 200);
        const variationName = compactLabel(event.template_variation_name, 160);
        if (subjectClusterId) out.subject_cluster_id = subjectClusterId;
        if (templateId) out.template_id = templateId;
        if (templateName) out.template_name = templateName;
        if (variationId) out.template_variation_id = variationId;
        if (variationName) out.template_variation_name = variationName;
        out.condition_count = finite(event.condition_count || 0, 100) || 0;
        out.conditions_matched = typeof event.conditions_matched === 'boolean'
          ? event.conditions_matched : null;
        out.conditions_enforced = event.conditions_enforced === true;
        const territoryId = compactLabel(event.account_territory_id, 64);
        const territoryName = compactLabel(event.account_territory_name, 160);
        const lastEmailedAt = finite(event.last_emailed_at, Number.MAX_SAFE_INTEGER, 1);
        if (territoryId && territoryId !== '0') out.account_territory_id = territoryId;
        if (territoryName && !/^(?:not set|select)$/i.test(territoryName)) {
          out.account_territory_name = territoryName;
        }
        if (lastEmailedAt) out.last_emailed_at = lastEmailedAt;
      }
      out.ok = event.ok !== false;
      return out;
    }
    if (kind === 'latency') {
      // REJECTED above the cap, not clamped to it. `finite` clamps, which is
      // right for a dwell time (a long read is a long read) and wrong here: a
      // 25s notification hold clamped to 5,000 is a five-second backend call
      // that never happened, recorded as if it had. Those samples then sat in
      // a dense line at exactly the ceiling and dragged every p95 and p99
      // above it — inflating the one chart the cap exists to protect.
      const raw = Math.round(Number(event.ms));
      if (!Number.isFinite(raw) || raw < 0 || raw > LATENCY_MS_MAX) return null;
      out.ms = raw;
      out.ok = event.ok !== false;
      return out;
    }
    const surface = String(event.surface || '').trim().slice(0, SURFACE_MAX);
    if (!surface) return null;
    out.surface = surface;
    out.surface_kind = SURFACE_KINDS.has(event.surface_kind) ? event.surface_kind : 'modal';
    if (kind === 'surface_close') {
      const ms = finite(event.ms, MS_MAX);
      if (ms === null) return null;
      out.ms = ms;
    }
    return out;
  }

  function storageRead(store) {
    if (!store?.get) return Promise.resolve(null);
    return new Promise((resolve) => {
      try {
        store.get([STATE_KEY], (data) => {
          void root.chrome?.runtime?.lastError;
          resolve(data?.[STATE_KEY] || null);
        });
      } catch { resolve(null); }
    });
  }

  function storageWrite(store, value) {
    if (!store?.set) return Promise.resolve();
    return new Promise((resolve) => {
      try {
        store.set({ [STATE_KEY]: value }, () => {
          void root.chrome?.runtime?.lastError;
          resolve();
        });
      } catch { resolve(); }
    });
  }

  function createReporter({
    chromeApi = root.chrome,
    auth = root.GBInstallationAuth,
    clock = () => Date.now(),
    newId = () => root.crypto.randomUUID(),
  } = {}) {
    if (!chromeApi?.alarms || !auth?.apiJson) {
      throw new Error('Usage telemetry is unavailable');
    }
    const store = chromeApi.storage?.session || null;
    let sessionId = String(newId());
    let startedAt = Number(clock()) || Date.now();
    let installationId = null;
    let buffer = [];
    let featureBuckets = new Map();
    let dropped = 0;
    let pendingBatch = null;
    let flushing = null;
    let soonTimer = null;
    let hydrated = false;
    let dirty = false;
    let writing = null;

    const featureKey = (event) => [
      event.feature,
      event.source,
      event.transport || '',
      event.ok === false ? '0' : '1',
    ].join('|');

    function mergeFeature(event) {
      const key = featureKey(event);
      const current = featureBuckets.get(key);
      if (current) {
        current.count = Math.min(COUNT_MAX, current.count + event.count);
        current.word_count = Math.min(COUNT_MAX, current.word_count + event.word_count);
        current.attachment_count = Math.min(current.count, current.attachment_count + event.attachment_count);
        current.inline_image_count = Math.min(current.count, current.inline_image_count + event.inline_image_count);
        current.at = Math.max(current.at || 0, event.at || 0);
        return;
      }
      featureBuckets.set(key, { ...event });
      if (featureBuckets.size > FEATURE_CAPACITY) {
        const oldest = featureBuckets.keys().next().value;
        featureBuckets.delete(oldest);
        dropped += 1;
      }
    }

    function pushBuffered(event) {
      buffer.push(event);
      if (buffer.length > CAPACITY) {
        dropped += buffer.length - CAPACITY;
        buffer = buffer.slice(-CAPACITY);
      }
    }

    function stateSnapshot() {
      return {
        version: STATE_VERSION,
        installation_id: installationId,
        session_id: sessionId,
        started_at: startedAt,
        dropped,
        events: buffer,
        features: [...featureBuckets.values()],
        pending_batch: pendingBatch,
      };
    }

    function queuePersist() {
      dirty = true;
      if (!hydrated || !store || writing) return;
      writing = (async () => {
        while (dirty) {
          dirty = false;
          await storageWrite(store, stateSnapshot());
        }
      })().finally(() => {
        writing = null;
        if (dirty) queuePersist();
      });
    }

    const ready = (async () => {
      let status = null;
      try { status = await auth.getStatus?.(); } catch { status = null; }
      installationId = status?.installationId || null;
      const saved = await storageRead(store);
      const sameInstallation = !saved?.installation_id || !installationId
        || saved.installation_id === installationId;
      if (saved && [2, STATE_VERSION].includes(saved.version) && sameInstallation) {
        sessionId = String(saved.session_id || sessionId);
        startedAt = Number(saved.started_at) || startedAt;
        dropped += finite(saved.dropped || 0, COUNT_MAX) || 0;
        if (saved.version === STATE_VERSION && saved.pending_batch?.batch_id) {
          pendingBatch = saved.pending_batch;
        }
        const currentEvents = buffer;
        buffer = [];
        for (const item of [...(Array.isArray(saved.events) ? saved.events : []), ...currentEvents]) {
          const normalized = normalize(item);
          if (!normalized || (normalized.kind === 'feature' && normalized.feature !== 'email_send')) continue;
          normalized.at = finite(item.at, Number.MAX_SAFE_INTEGER) || (Number(clock()) || Date.now());
          pushBuffered(normalized);
        }
        const currentFeatures = [...featureBuckets.values()];
        featureBuckets = new Map();
        for (const item of [...(Array.isArray(saved.features) ? saved.features : []), ...currentFeatures]) {
          const normalized = normalize(item);
          if (!normalized || normalized.kind !== 'feature') continue;
          normalized.at = finite(item.at, Number.MAX_SAFE_INTEGER) || (Number(clock()) || Date.now());
          if (normalized.feature === 'email_send') pushBuffered(normalized);
          else mergeFeature(normalized);
        }
      }
      if (buffer.length > CAPACITY) {
        dropped += buffer.length - CAPACITY;
        buffer = buffer.slice(-CAPACITY);
      }
      hydrated = true;
      queuePersist();
    })();

    function record(event) {
      const normalized = normalize(event);
      if (!normalized) return false;
      normalized.at = Number(clock()) || Date.now();
      if (normalized.kind === 'feature') {
        if (normalized.feature === 'email_send') pushBuffered(normalized);
        else mergeFeature(normalized);
      } else {
        pushBuffered(normalized);
      }
      queuePersist();
      return true;
    }

    function sample(ms, ok) {
      return record({ kind: 'latency', ms, ok });
    }

    async function flush() {
      if (flushing) return flushing;
      flushing = (async () => {
        await ready;
        if (writing) await writing;
        if (!pendingBatch) {
          pendingBatch = {
            batch_id: String(newId()),
            session_id: sessionId,
            started_at: startedAt,
            extension_version: String(chromeApi.runtime?.getManifest?.().version || '').slice(0, 40),
            dropped,
            events: [...buffer, ...featureBuckets.values()],
          };
          buffer = [];
          featureBuckets = new Map();
          dropped = 0;
          // Persist the outbox before the request. A worker eviction or a
          // timeout after the server commits must replay this exact batch id.
          queuePersist();
          if (writing) await writing;
        }
        const batch = pendingBatch;
        try {
          const acknowledgement = await auth.apiJson(ADDRESS, {
            method: 'POST',
            body: JSON.stringify(batch),
            responseLimit: 8 * 1024,
          });
          if (acknowledgement?.ok !== true
              || acknowledgement.batch_id !== batch.batch_id
              || acknowledgement.accepted !== batch.events.length) {
            throw new Error('Telemetry acknowledgement did not match the pending batch');
          }
          pendingBatch = null;
          queuePersist();
          if (writing) await writing;
          return true;
        } catch {
          // Keep one bounded batch in the durable outbox. Product work remains
          // non-blocking, while the next alarm safely retries the same batch.
          return false;
        }
      })();
      try {
        return await flushing;
      } finally {
        flushing = null;
      }
    }

    function flushSoon() {
      if (soonTimer) return;
      soonTimer = setTimeout(() => {
        soonTimer = null;
        flush().catch(() => {});
      }, SOON_MS);
    }

    function start() {
      try {
        chromeApi.alarms.create(ALARM, { periodInMinutes: PERIOD_MINUTES });
      } catch { /* alarms unavailable — rare events may still flush soon */ }
    }

    chromeApi.alarms.onAlarm?.addListener((alarm) => {
      if (alarm?.name === ALARM) flush().catch(() => {});
    });

    return Object.freeze({
      get sessionId() { return sessionId; },
      record,
      sample,
      flush,
      flushSoon,
      start,
      ready: () => ready,
      pending: () => (pendingBatch?.events?.length || 0) + buffer.length + featureBuckets.size,
    });
  }

  root.GBUsageTelemetry = Object.freeze({
    createReporter,
    normalize,
    ADDRESS,
    ALARM,
    STATE_KEY,
    CAPACITY,
    FEATURE_CAPACITY,
  });
})(globalThis);

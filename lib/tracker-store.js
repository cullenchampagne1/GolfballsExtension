...
(function installTrackerStore(root) {
  ...
  /* ── scheduler state ───────────────────────────────────────────────
     When each tracker last completed a list poll, how far its DATA has been
     read, and whether the rep has turned that tracker off. Kept apart from the
     records so a sweep that finds nothing new still writes ~50 bytes rather
     than rewriting the whole table — and so turning a tracker off never
     touches the rows it already collected. */
  ...
  async function patchState(trackerId, patch) {
    const current = await state();
    const next = {
      ...current,
      [trackerId]: { ...(current[trackerId] || {}), ...patch },
    };
    await write({ [STATE_KEY]: next });
    return next;
  }

  /** The SCHEDULE clock: when this tracker last ran a list poll. */
  async function markPolled(trackerId, at = Date.now()) {
    return patchState(trackerId, { lastPolledAt: at });
  }
  ...
  /* ── the data cursor ───────────────────────────────────────────────
     TWO CLOCKS, NOT ONE. `lastPolledAt` answers "when did this tracker last
     run", and it moves on every attempt because that is what paces the next
     one. `cursorAt` answers a completely different question — "how far has
     this tracker's data actually been READ" — and it moves only when a sweep
     read a whole window and stored what was in it.

     They were the same field once, and that is how the recent-orders table
     stayed empty: a sweep that reached the index but recorded nothing (rows it
     could not map, a half-read page set) still stamped the clock, so the next
     sweep asked for a NARROWER window and the orders in the gap were never
     asked about again. A cursor that only advances over data it actually
     banked cannot skip: a sweep that learns nothing simply asks again.

     It is also the tracker's OWN cursor. Nothing a rep does by hand — the
     "Scan for recent orders" quick action keeps its own watermark under
     `gbScanRecentOrders_lastRun` — touches this key. */

  async function markCursor(trackerId, at = Date.now()) {
    const stamp = Number(at) || 0;
    const current = await cursorAt(trackerId);
    // Monotonic: a cursor that could move backwards would re-read a window
    // forever, and one that could be pushed forward by a stale sweep would
    // skip. Only forward, only past data we hold.
    if (stamp <= current) return null;
    return patchState(trackerId, { cursorAt: stamp });
  }

  async function cursorAt(trackerId) {
    const current = await state();
    return Number(current?.[trackerId]?.cursorAt) || 0;
  }

  /** Rewind a tracker's data cursor to its first-run lookback. The repair for
   *  a tracker that swept over a window it could not read at the time. */
  async function resetCursor(trackerId) {
    return patchState(trackerId, { cursorAt: 0 });
  }
  ...
  async function snapshot(trackerId, { records: withRecords = true } = {}) {
    ...
    const summary = {
      ...
      cursorAt: Number(stateBag?.[tracker.id]?.cursorAt) || 0,

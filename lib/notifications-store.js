...
(function installNotificationsStore(root) {
  ...
  function normalizeAction(value) {
    ...
    if (type === 'open_mockup_batch') {
      ...
      if (!/^batch_[a-f0-9]{32}$/.test(batchId)) return null;
      return {
        type, batchId, label: safeText(value.label || 'View batch', 48),
      };
    }
    if (type === 'open_contact') {
      const contactEmail = String(value.contact_email || '').trim().toLowerCase();
      if (!/^[^@\s]{1,128}@[^@\s]{1,190}$/.test(contactEmail)) return null;
      return {
        type,
        contactEmail,
        messageId: safeText(value.message_id, 180),
        label: safeText(value.label || 'Open contact', 48),
      };
    }
    if (type === 'open_support_ticket') {
      const ticketId = String(value.ticket_id || '');
      if (!/^GBT-[A-Z0-9]{6,16}$/.test(ticketId)) return null;
      return {
        type, ticketId, label: safeText(value.label || 'View ticket', 48),
      };
    }
    return null;
  }

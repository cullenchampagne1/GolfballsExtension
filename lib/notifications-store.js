...
(function installNotificationsStore(root) {
  ...
  function normalizeAction(value) {
    return root.GBNotificationActions?.normalize?.(value) || null;
  }

  function normalizePresentation(value) {
    const raw = value && typeof value === 'object' ? value : {};
    const delivery = ['native', 'page', 'both', 'center'].includes(
      String(raw.delivery || ''),
    ) ? String(raw.delivery) : 'native';
    return {
      delivery,
      requireInteraction: raw.require_interaction === true
        || raw.requireInteraction === true,
    };
  }

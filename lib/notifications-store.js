...
(function installNotificationsStore(root) {
  ...
  // Every cache mutation is a read-modify-write transaction. Chrome storage
  // does not make overlapping transactions atomic, so rapid archive clicks
  // must line up behind one another instead of letting the last writer restore
  // every row the earlier writers changed.
  ...
  async function mergeRemote(values) {
    ...
    return serializeMutation(async () => {
      ...
      for (const item of incoming) {
        ...
        if (previous) {
          const status = previous.status === 'dismissed' || item.status === 'dismissed'
            ? 'dismissed'
            : (previous.status === 'read' || item.status === 'read' ? 'read' : 'unread');
          byRemote.set(item.remoteId, {
            ...
            deliveredAt: item.deliveredAt || previous.deliveredAt || null,
            readAt: item.readAt || previous.readAt || null,
            dismissedAt: item.dismissedAt || previous.dismissedAt || null,
            actedAt: item.actedAt || previous.actedAt || null,

...
(function installAnonymousApiClient(root) {
  ...
  const NOTIFICATION_POLL_PATH = `${CLIENT_BASE}/notifications`;
  const TRANSPORT_PATHS = new Set([
    NOTIFICATION_POLL_PATH,
    `${NOTIFICATION_POLL_PATH}/receipts`,
    `${CLIENT_BASE}/telemetry`,
  ]);
  // The email-relay system service, reachable only as the single by-reference
  // message lookup an "Open email" notification action needs.
  const RELAY_MESSAGES_PATH = '/services/email-relay-service/messages';
  const RESPONSE_LIMIT = 32_000;
  // Default transport safety guard for ordinary extension JSON calls. Durable
  // share routes are explicitly exempt below because their contract has no
  // byte quota; all other authenticated calls remain bounded.
  ...
  const cooldownUntil = { interactive: 0, transport: 0 };
  ...
  function isAllowedApiRoute(url, method) {
    ...
    if (url.pathname.startsWith(`${CLIENT_BASE}/`)) {
      ...
      if (method === 'PATCH') {
        return !url.search && new RegExp(
          `^${CLIENT_BASE}/email-template-shares/[A-Za-z0-9_-]{32}$`,
        ).test(url.pathname);
      }
      if (method === 'PUT') {
        return !url.search && url.pathname === `${CLIENT_BASE}/email-template-bucket`;
      }
      return ['GET', 'POST'].includes(method);
    }
    // One relayed email, named by the 32-hex reference its notification action
    // carried. Deliberately not a general read of the mailbox mirror: no
    // contact search, no cursor paging, no other query key.
    if (url.pathname === RELAY_MESSAGES_PATH) {
      if (method !== 'GET') return false;
      const params = url.searchParams;
      return [...params.keys()].every((key) => ['ref', 'limit'].includes(key))
        && /^[a-f0-9]{32}$/.test(params.get('ref') || '')
        && ['', '1'].includes(params.get('limit') ?? '');
    }
  ...
  function isUnboundedShareRoute(url) {
    const path = url.pathname;
    return path === `${CLIENT_BASE}/email-template-bucket`
      || [`settings-shares`, `email-template-shares`, `product-stores`]
        .some((kind) => path === `${CLIENT_BASE}/${kind}`
          || path.startsWith(`${CLIENT_BASE}/${kind}/`));
  }

  function shouldSampleLatency(url, method) {
    // A notification GET is a cursor transport the server deliberately holds
    // open. Its wall-clock duration is wait budget, not work the user waited
    // for. Notification receipts remain sampled because they are ordinary
    // POSTs, as do all other extension API requests.
    if (method === 'GET' && url.pathname === NOTIFICATION_POLL_PATH) return false;
    // The telemetry flush itself must never create another telemetry sample,
    // including when its fetch rejects before returning a response.
    return !root.GBUsageTelemetry
      || url.pathname !== root.GBUsageTelemetry.ADDRESS;
  }

  function requestLane(url) {
    return TRANSPORT_PATHS.has(url.pathname) ? 'transport' : 'interactive';
  }

  function retryAfterSeconds(response, fallback = 1) {
  ...
  function enforceCooldown(lane) {
    const remaining = Math.ceil((cooldownUntil[lane] - Date.now()) / 1000);
    if (remaining <= 0) return;
    const error = new Error(`Extension API ${lane} requests are rate limited; retry after ${remaining}s`);
    error.status = 429;
  ...
  async function apiFetch(path, options = {}) {
    ...
    if (options.body != null && typeof options.body !== 'string') {
      throw new Error('Extension API body must be a serialized string');
    }
    if (!isUnboundedShareRoute(url) && options.body != null
        && options.body.length > REQUEST_BODY_LIMIT) {
    ...
    const lane = requestLane(url);
    enforceCooldown(lane);
    const installation = await ensureInstallation();
    const headers = new Headers(options.headers || {});
    headers.delete('authorization');
    headers.set('Authorization', `Bearer ${installation.apiKey}`);
    headers.set('Accept', 'application/json');
    // The Latency block's p50/p95/p99 are measured HERE, around the real
    // network call, because that is the wait an installation actually
    // experiences. Timing it on the server would report the backend's own
    // handler time and miss every hop between them. The telemetry reporter
    // installs `GBUsageSink`; without it (tests, the enrollment path before
    // the worker finishes booting) this is a no-op.
    const sampleLatency = shouldSampleLatency(url, method);
    const startedAt = sampleLatency ? Date.now() : 0;
    const sink = root.GBUsageSink;
    let response;
    try {
      ...
      // A failed request is still a wait the user sat through, so it belongs
      // in the percentiles — flagged, so the card can say so.
      if (sampleLatency) {
        try { sink?.(Date.now() - startedAt, false); } catch { /* never fail a request for telemetry */ }
      }
      throw error;
    }
    if (sampleLatency) {
      try { sink?.(Date.now() - startedAt, response.ok); } catch { /* as above */ }
    }
    if (response.status === 429) {
      cooldownUntil[lane] = Math.max(
        cooldownUntil[lane], Date.now() + retryAfterSeconds(response) * 1000,
      );
    }
    const decisionPath = root.GBRuntimeBootstrap?.isDecisionPath?.(url.pathname) === true;
    if (response.status === 401) {
      // The health response body owns the access decision.  Closing here would
      // turn any proxy/scope/auth 401 into a persisted "Access paused" state
      // before runtime-bootstrap can classify it as transient.
      if (!decisionPath) {
  ...
  async function fetchConfiguration() {
    const installation = await ensureInstallation();
    // Version negotiation lets the server preserve the exact registry shape
    // required by already-published clients while current builds receive the
    // current policy.  Builds through 3.4.2 sent no version and are treated as
    // the legacy contract by the server.
    const version = encodeURIComponent(runtimeIdentity().extensionVersion);
    const response = await fetch(
      `${API_ORIGIN}${CLIENT_BASE}/configuration?extension_version=${version}`,
      {
        method: 'GET',
        headers: {
  ...
  async function apiJson(path, options = {}) {
    const requestUrl = new URL(String(path || ''), API_ORIGIN);
    const responseLimit = isUnboundedShareRoute(requestUrl)
      ? Infinity
      : (Number.isFinite(Number(options.responseLimit))
  ...
  function withResponseMetadata(error, response) {
    const seconds = retryAfterSeconds(response, 0);
    if (seconds > 0) error.retryAfterSeconds = seconds;
    return error;
  }
  ...
  function formatApiErrorDetail(payload) {
    ...
    if (payload?.detail && typeof payload.detail === 'object' && !Array.isArray(payload.detail)) {
      return String(payload.detail.message || '').trim().slice(0, 240);
    }

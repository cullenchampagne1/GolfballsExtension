...
(function installAnonymousApiClient(root) {
  ...
  async function apiFetch(path, options = {}) {
    ...
    if (response.status === 401) {
      root.GBExtensionAccessGateController?.disable?.(
        'credential-or-access-disabled', { reload: true },
      ).catch?.(() => {});
    } else if (response.status === 403 && url.pathname.startsWith(`${CLIENT_BASE}/`)) {
      if (url.pathname === `${CLIENT_BASE}/health`) {
        root.GBExtensionAccessGateController?.disable?.(
          'credential-or-access-disabled', { reload: true },
        ).catch?.(() => {});
      } else {
        // A resource-specific 403 is not proof that the whole installation was
        // disabled. Ask the dedicated health endpoint for that decision.
        root.GBExtensionAccessGateController?.check?.().catch?.(() => {});
      }
    }
    return response;
  }
  ...
  chrome.storage.onChanged?.addListener((changes, area) => {
    ...
    if (next && next !== previous) {
      if (root.GBExtensionAccessGateController?.isEnabled?.() !== true) return;

...
(function installActionLanguage(root) {
  ...
  const COMMANDS = Object.freeze([
    ...
    'open_support_ticket',
    /* @admin:start */
    /* Admin-only command, excluded from the served build two ways because this
       file is loaded both raw (worker/content script — the markers strip it)
       and bundled into React surfaces (where __ADMIN__ is a build literal and
       the whole entry is dead-code-eliminated). Raw loads have no __ADMIN__ at
       all, which is the admin/dev install. */
    ...((typeof __ADMIN__ === 'undefined' ? true : __ADMIN__)

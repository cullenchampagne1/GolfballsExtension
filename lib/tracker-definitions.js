  ...
  const recentOrders = {
    ...
    poll: {
      ...
      collect: async ({ crmContacts, since, now, log = () => {} }) => {
        ...
        // How far the cursor may move when the read was TRUNCATED. The search
        // is newest-first, so a capped read drains the NEW end of the window
        // completely and leaves the old end unread: everything from the oldest
        // row we banked up to now was seen in full. Without this, a rep whose
        // window holds more contacts than five pages can carry (a book of a
        // few thousand puts a week over the ceiling easily) truncates on every
        // sweep, so the cursor never moves off its first-run lookback and the
        // window only widens — permanently stuck, by a rule meant to protect
        // the data. The rows below that floor are older than 500 newer orders
        // and could never reach a 300-row table anyway.
        const bankedAt = rows.map((row) => Number(row.at)).filter(Number.isFinite);
        return {
          rows,
          seen: docs.length,
          numFound: Array.isArray(answer) ? docs.length : answer?.numFound,
          complete: Array.isArray(answer) ? true : answer?.complete !== false,
          cursorTo: bankedAt.length ? Math.min(...bankedAt) : 0,
        };
      },
    },

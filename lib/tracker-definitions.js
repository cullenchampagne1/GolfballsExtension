...
(function installTrackerDefinitions(root) {
  ...
  /* ── recent orders ────────────────────────────────────────────────
     ...
     name is likewise only authoritative on a CRM page. So `collect` is handed
  ...
  /** One CRM Search contact row → the order it tells us that contact placed. */
  function orderFromContactDoc(doc, { now = Date.now() } = {}) {
    ...
    if (String(pick(doc, ['recordType_s']) ?? '').toLowerCase() !== 'contact') return null;
    const contactId = contactIdFromDoc(doc);
    const orderedAt = Date.parse(String(pick(doc, ['lastOrderDate_dt']) ?? ''));
    if (!contactId || !Number.isFinite(orderedAt)) return null;
  ...
  const recentOrders = {
    ...
    poll: {
      everyMinutes: 15,
      collect: async ({ crmContacts, since, now }) => {
        if (typeof crmContacts !== 'function') {
          throw new Error('recent orders need a CRM page to search from');
        }
        const docs = await crmContacts({ since, now });
        return (Array.isArray(docs) ? docs : [])
          .map((doc) => orderFromContactDoc(doc, { now }))
          .filter(Boolean);
      },
    },
  ...
  root.GBTrackerDefinitions = Object.freeze({
    ...
    // exercise it read a contact row exactly as the sweep does.
    contactIdFromDoc,
    orderFromContactDoc,
  });
})(globalThis);

  ...
  async function applyAll(notifications) {
    ...
    for (const notification of rows) {
      ...
      try {
        ...
        // Exhausted retries: log and skip this one row instead of holding the
        // cursor — and every notification behind it — hostage indefinitely.
        console.error(
          `[GBLiveUpdates] giving up on notification ${id || '(unknown)'} after ${attempts} attempt(s)`,
          error,
        );
        if (id && failures[id]) {
          delete failures[id];

...
(function installLiveUpdates(root) {
  ...
  const EMAIL_SUBMISSION_REVISION_KEY = 'gbEmailTemplateSubmissionRevision';
  const APPLY_FAILURES_KEY = 'gbNotificationApplyFailures';
  ...
  // One notification whose authoritative refresh keeps throwing (a bad
  // config payload, a share that 404s, ...) must not wedge every later
  // notification behind it forever — including unrelated types, since the
  // cursor only advances once a whole batch applies clean. Retry a few
  // times across separate poll cycles for the ordinary transient case, then
  // give up on that one row and move on. Features with their own periodic
  // refresh alarm (remote-settings-policy, managed-email-templates) catch up
  // on their own cadence even when their live-update trigger is skipped.
  ...
  async function apply(notification) {
    ...
    if (event.type === 'email_templates.changed') {
      ...
      const policy = root.GBRemoteSettingsPolicy;
      if (!policy || typeof policy.sync !== 'function') {
        throw new Error('Remote settings policy synchronization is unavailable');
      }
      // Settings are authoritative state just like the managed bucket. A
      // failed refresh keeps this outbox row pending instead of requiring the
      // user to reopen Settings to trigger another request.
      await policy.sync({ force: true });
    } else if (event.type === 'managed_email_templates.changed') {
      const managed = root.GBManagedEmailTemplates;
      if (!managed || typeof managed.sync !== 'function') {
        throw new Error('Managed email-template synchronization is unavailable');
      }
      // This event is the bucket's invalidation transport. Let a failed fetch
      // reach the poller so it retains the cursor and retries this same row.
      await managed.sync({ force: true });
    } else if (event.type === 'email_template_submissions.changed') {
      writes[EMAIL_SUBMISSION_REVISION_KEY] = update;
      const submissions = root.GBEmailTemplateSubmissions;
      if (!submissions || typeof submissions.sync !== 'function') {
        throw new Error('Email-template submission synchronization is unavailable');
      }
      await submissions.sync({ force: true });
    }
  ...
  async function applyAll(notifications) {
    ...
    const bag = await getStorage(APPLY_FAILURES_KEY);
    const failures = { ...(bag[APPLY_FAILURES_KEY] || {}) };
    ...
    for (const notification of rows) {
      ...
      try {
        ...
        const attempts = (failures[id]?.attempts || 0) + 1;
        if (id && attempts < MAX_APPLY_ATTEMPTS) {
          failures[id] = { attempts, firstFailedAt: failures[id]?.firstFailedAt || Date.now() };
          await setStorage({ [APPLY_FAILURES_KEY]: failures });
          throw error;
        }
        // Exhausted retries: skip this one row instead of holding the cursor —
        // and every notification behind it — hostage indefinitely. Runtime
        // console output is deliberately avoided; the retry behavior is
        // observable through the persisted failure count until this point.
        if (id && failures[id]) {
          delete failures[id];
          failuresChanged = true;
        }
    ...
    if (failuresChanged) await setStorage({ [APPLY_FAILURES_KEY]: failures });
    return applied;
  }

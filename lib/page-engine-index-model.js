...
(function installPageEngineIndexModel(root) {
  ...
  const SUPPORTED_SCHEMAS = new Set(['account', 'contact', 'opportunity', 'order']);
  ...
  function normalizeOwnerId(value) {
    const owner = normalizeText(value, MAX_OWNER_CHARS);
    if (!owner) throw new Error('Engine Account Id is required');
    return owner.toLocaleLowerCase('en-US');
  }

  function ownerCandidates(data) {
    const page = object(data);
    const account = object(page.account);
    const order = object(page.order);
    return [...new Set([
      account.salesRepId,
      account.salesRep,
      order.salesRepId,
  ...
  function snapshotIdentity(snapshot) {
    ...
    const bySchema = {
      ...
      opportunity: { entityType: 'opportunity', id: ids.opportunity },
      order: { entityType: 'order', id: ids.order },
    };
    ...
    return {
      ...
      id,
      accountId: normalizeText(ids.account || (schemaId === 'order' ? data.order?.customerId : ''), MAX_ID_CHARS),
      contactId: normalizeText(ids.contact, MAX_ID_CHARS),
    };
  }

  function normalizeSnapshot(snapshot, configuredOwner) {
    const identity = snapshotIdentity(snapshot);
    const ownerId = normalizeOwnerId(configuredOwner);
    const data = object(snapshot.data);
    if (!ownerCandidates(data).includes(ownerId)) {
      throw new Error('Page owner does not match Engine Account Id');
    }
    return {
      ...identity,
      ownerId,

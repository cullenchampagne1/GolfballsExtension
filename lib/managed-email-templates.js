...
(function installManagedEmailTemplates(root) {
  ...
  const OMIT = new Set([
    'id', 'folderId', 'shareImport', 'shareSync', META_KEY,
  ...
  function reconcile(existing, payload, settings = {}, options = {}) {
    ...
    for (const item of items) {
      ...
      if (!previous && isParent) {
        previous = ordinary.find((row) => !row?.shareImport
          && String(row?.type || 'order') === String(item.template?.type || 'order')
          && String(row?.name || '').trim().toLowerCase()
            === String(item.template?.name || '').trim().toLowerCase()
          && same(serverTemplate(row), item.template));
        if (previous) ordinary.splice(ordinary.indexOf(previous), 1);
      }
      if (!previous) {
  ...
  function install() {
    chrome.storage?.onChanged?.addListener((changes, area) => {
      ...
      if (changes.templates && !changes[CACHE_KEY] && !applying) {
        getStorage('devSettings').then((stored) => {
          if (capabilities(stored.devSettings).isParent) schedulePublish();
        });
      }
    });
  }
  ...
  root.GBManagedEmailTemplates = Object.freeze({
    CACHE_KEY, META_KEY, KIND, metadata, serverTemplate, reconcile, writes, needsPublish,

...
(function installManagedEmailTemplates(root) {
  ...
  // Template autosaves and remote invalidations can both request a publish.
  // Keep the network writes ordered so two snapshots from this installation
  // can never race each other at the server and manufacture a self-conflict.
  ...
  function reconcile(existing, payload, settings = {}, options = {}) {
    ...
    for (const item of items) {
      ...
      let priorMeta = metadata(previous);
      if (priorMeta && options.acknowledgedBucketIds?.has(String(item.id))) {
        // The server accepted the snapshot sent by this publish. Advance the
        // local merge base to that authoritative result, but keep any newer
        // editor state that landed in storage while the request was in flight.
        priorMeta = {
          ...priorMeta,
          version: Math.max(1, Number(item.version) || 1),
          snapshot: clone(item.template || {}),
          lastEditor: String(item.last_editor || priorMeta.lastEditor || 'Management'),
          conflictWith: Array.isArray(item.conflict_with) ? item.conflict_with.map(String) : [],
          updatedAt: String(item.updated_at || priorMeta.updatedAt || ''),
        };
        delete priorMeta.remoteVersion;
        delete priorMeta.remoteSnapshot;
        previous = { ...previous, [META_KEY]: priorMeta };
      }
      const preserveDirty = !options.acceptRemote
        || options.preserveLocalBucketIds?.has(String(item.id));
      const dirtyParent = preserveDirty && isParent && priorMeta
        && !same(serverTemplate(previous), priorMeta.snapshot);
      if (dirtyParent) {
        // A locally-dirty row (still typing, not yet published) is not
        // itself a conflict — every accepted publish, including this
        // installation's own, bumps the bucket row's version, so
        // "remote version moved past what I last knew" is a routine part of
        // solo editing, not evidence someone else touched it. The bucket's
        // conflict_with is computed server-side from live peer rows sharing
        // this template's type+name and is the only authoritative signal;
        // mirror it every reconcile (dirty or not) instead of accumulating
        // a local guess that only ever grows, which is what left the
        // "Conflict with …" badge pinned on screen long after the server
        // stopped reporting any conflict.
  ...
  async function publishOnce() {
    ...
    const update = writes(stored.templates, stored[CACHE_KEY]);
    const payload = await api.apiJson(PATH, {
      method: 'PUT',
      body: JSON.stringify(update),
    });
    // Do not reconcile against `stored`: the request may have spent long
    // enough in flight for the editor to save a newer complete HTML document.
    // Re-read storage and preserve only rows that actually changed after their
    // submitted snapshot. Using the stale pre-request object here was the path
    // that could replace a finished template with an intermediate value such
    // as a lone "&".
    const latest = await getStorage(['templates', 'devSettings', CACHE_KEY]);
    const sentByBucket = new Map(update.templates
      .filter((row) => row.bucket_id)
      .map((row) => [String(row.bucket_id), row.template]));
    const conflictIds = new Set((Array.isArray(payload?.sync_conflicts)
      ? payload.sync_conflicts : []).map((row) => String(row?.bucket_id || '')));
    const preserveLocalBucketIds = new Set(conflictIds);
    for (const template of (Array.isArray(latest.templates) ? latest.templates : [])) {
      const meta = metadata(template);
      const sent = meta && sentByBucket.get(String(meta.bucketId));
      if (sent && !same(serverTemplate(template), sent)) {
        preserveLocalBucketIds.add(String(meta.bucketId));
      }
    }
    const acknowledgedBucketIds = new Set(
      [...sentByBucket.keys()].filter((bucketId) => !conflictIds.has(bucketId)),
    );
    await applyPayload(payload, latest, {
      acceptRemote: true,
      preserveLocalBucketIds,
      acknowledgedBucketIds,
    });
    return payload;
  }

  function publish() {
    const task = publishQueue.then(publishOnce, publishOnce);
    // A failed pass must reject its own caller while leaving the queue usable
    // for the scheduled retry and later editor saves.

...
(function installEmailRelayPoll(root) {
  ...
  // Admin-only feature: poll the standalone email-relay SERVICE directly (the
  // golfballs project no longer bridges it). The service gates this on the
  // client:email-relay scope, which the installation key carries. The service's
  // HTTP route is a short poll (no long-poll wait); the `wait` param is ignored
  // and the alarm/loop cadence below drives near-real-time delivery.
  const ENDPOINT = '/services/email-relay-service/messages/pending';

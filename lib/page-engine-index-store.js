 ...
 * non-extractable key they have no field name, record id, or owner identity.
 */
(function installPageEngineIndexStore(root) {
  ...
  async function deriveKeys(ownerId) {
    const ownerAccount = Model.normalizeOwnerId(ownerId);
    const stored = await deviceKeys();
    return {
      encryptionKey: stored.encryptionKey,
      lookupKey: stored.lookupKey,
      keyVersion: stored.keyVersion,
      owner: await hmac(stored.lookupKey, `engine-owner|${ownerAccount}`),
    };
  }
  ...
  async function currentConfig() {
    ...
    return {
      ...
      ownerId: Model.normalizeText(settings['pageEngine.accountId'], 200),
    };
  }
  ...
  async function indexSnapshotInternal(input) {
    ...
    if (!config.ownerId) return { indexed: false, reason: 'missing-account-id' };
    const snapshot = Model.normalizeSnapshot(input, config.ownerId);
    const keys = await deriveKeys(config.ownerId);
  ...
  async function query(input = {}) {
    const config = await currentConfig();
    if (!config.ownerId) return { rows: [], total: 0, matched: 0, scanned: 0 };
    const keys = await deriveKeys(config.ownerId);
  ...
  async function stats() {
    const config = await currentConfig();
    if (!config.ownerId) return { total: 0, byType: {}, lastIndexedAt: null };
    const keys = await deriveKeys(config.ownerId);
    const records = await allOwnerRecords(keys);
    const byType = {};
  ...
  async function clear() {
    const config = await currentConfig();
    if (!config.ownerId) return { cleared: 0 };
    const keys = await deriveKeys(config.ownerId);
    const records = await allOwnerRecords(keys);
    await deleteRecordKeys(records.map((record) => record.storageKey));

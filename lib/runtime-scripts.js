(function installRuntimeScripts(root) {
  ...
  function invoke(chromeApi, fn, ...args) {
    return new Promise((resolve, reject) => fn(...args, (result) => {
      const failure = chromeApi.runtime?.lastError;
      if (failure) reject(new Error(failure.message || 'Chrome API request failed'));
      else resolve(result);
    }));
  }
  ...
  function createManager(chromeApi = root.chrome) {
    ...
    async function closeSurfaces() {
      let tabs = [];
      try {
        tabs = await invoke(chromeApi, chromeApi.tabs.query.bind(chromeApi.tabs), {});
      } catch {
        return;
      }
      ...
      try {
        await invoke(chromeApi, chromeApi.tabs.remove.bind(chromeApi.tabs), selected);
      } catch {
        // Best effort.
      }
    }
    ...
    async function reloadTabs() {
      ...
      await Promise.all(tabs.map(async (tab) => {
        ...
        try {
          await invoke(chromeApi, chromeApi.tabs.reload.bind(chromeApi.tabs), tab.id);
        } catch {
          // Best effort.
        }
      }));
    ...
    async function open() {
      ...
      await invoke(chromeApi, chromeApi.action.enable.bind(chromeApi.action));
      openState = true;
      return true;
    }
    ...
    async function close({ reload = false, transitioned = true } = {}) {
      ...
      try {
        await invoke(chromeApi, chromeApi.action.disable.bind(chromeApi.action));
      } catch {
        // Best effort.
      }
      await closeSurfaces();

...
(function installNotificationPoll(root) {
  ...
  async function processResult(result) {
    ...
    // disabled visible notifications. A malformed/future event cannot block
    // receipts, visible rows, or later cursor entries.
    try { await root.GBLiveUpdates?.applyAll?.(result.notifications); }
    catch { /* each authoritative surface also refreshes on open */ }

...
(function installNotificationPoll(root) {
  ...
  async function enrichActions(items) {
    ...
    await Promise.all(items.map(async (item) => {
      if (item?.action?.type !== 'open_contact') return;
      const url = await resolveContact(
        item.action.arguments?.contact_email,
      );
      if (!url) return;
  ...
  async function processResult(result) {
    ...
    for (const item of added) {
      const delivery = item.presentation?.delivery || 'native';
      if (delivery === 'native' || delivery === 'both') {
        await root.GBNativeNotifications?.show?.(item);
      }
    }
    const pageItems = added.filter((item) => (
      item.presentation?.delivery === 'page'
      || item.presentation?.delivery === 'both'
    ));
    if (!pageItems.length) return true;
    const tab = await activeGolfballsTab();
    if (!tab) return true;
    for (const item of pageItems.slice(0, MAX_TOASTS)) {
      sendToTab(tab, {
        action: 'GB_EXTENSION_NOTIFICATION',
        notification: item,
      });
    }
    const overflow = pageItems.length - MAX_TOASTS;
    if (overflow > 0) {
      sendToTab(tab, {
        action: 'GB_NOTIFY',
        type: 'info',

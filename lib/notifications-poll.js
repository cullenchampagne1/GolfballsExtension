...
(function installNotificationPoll(root) {
  ...
  const pendingReceiptGroups = new Map();
  ...
  function receiptIds(notificationIds) {
    return [...new Set(
  ...
  async function postReceipts(notificationIds, state) {
    const auth = root.GBInstallationAuth;
    if (!auth || typeof auth.apiJson !== 'function') return false;
    try {
      for (let offset = 0; offset < notificationIds.length; offset += RECEIPT_BATCH_SIZE) {
        await auth.apiJson(RECEIPTS_ENDPOINT, {
          method: 'POST',
          body: JSON.stringify({
            notification_ids: notificationIds.slice(offset, offset + RECEIPT_BATCH_SIZE),
            state,
          }),
        });
      }
  ...
  async function flushReceipts() {
    ...
    const groups = [...pendingReceiptGroups.values()];
    pendingReceiptGroups.clear();
    for (const group of groups) {
      const sent = await postReceipts([...group.ids], group.state);
      group.resolve.forEach((done) => done(sent));
    }
  }

  function sendReceipt(notificationIds, state) {
    const ids = receiptIds(notificationIds);
    const auth = root.GBInstallationAuth;
    if (!ids.length || !auth || typeof auth.apiJson !== 'function') {
      return Promise.resolve(false);
    }
    let group = pendingReceiptGroups.get(state);
    if (!group) {
      group = { state, ids: new Set(), resolve: [] };
      pendingReceiptGroups.set(state, group);
    }
    ids.forEach((id) => group.ids.add(id));
    const result = new Promise((resolve) => group.resolve.push(resolve));
    if (receiptFlushTimer === null) {

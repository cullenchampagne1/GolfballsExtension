/** Zero-setup encrypted CRM index worker.
 *
 * The whole suite shares one fake IndexedDB, and each step depends on the state
 * the previous one left behind, so these `it` blocks run in declaration order.
 */
...
const db = {
  ...
  transaction(name) {
    const tx = {
      objectStore: (storeName) => stores.get(storeName || name),
...
before(async () => {
  ...
  globalThis.chrome = { runtime: { id: 'annoeoeiijgdgmlpefllibcilcamnjek' } };
  await import(`../../lib/crm-index-store.js?test=${Date.now()}`);
});

describe('encrypted CRM index', () => {
  let firstStorageKey;

  it('encrypts indexed rows and persists non-extractable device keys', async () => {
  ...
  it('leaves no plaintext identifiers, contact details, or revenue at rest', () => {
  ...
  it('decrypts only the matching rows for the owning employee', async () => {
  ...
  it('returns nothing for a different employee', async () => {
    const other = await globalThis.GBCrmIndex.search({ query: '', limit: 8, employeeId: '456' });
    assert.equal(other.total, 0);
    assert.deepEqual(other.rows, []);
  });

  it('reopens the same keys after a service-worker restart without setup', async () => {
  ...
  it('deletes a single record by its plaintext id', async () => {
    firstStorageKey = records.keys().next().value;
    await globalThis.GBCrmIndex.deleteRecord('contact_42', '123');
    assert.equal(records.has(firstStorageKey), false);
  });

  it('clears records written by an older schema automatically', async () => {
  ...
  it('clears ciphertext and regenerates keys when the local key is missing', async () => {
    keys.clear();
  ...
  it('keeps the device keys when the index is cleared', async () => {
...
describe('encrypted CRM index · setup and client guardrails', () => {
  it('requires no managed storage policy', async () => {
    const manifest = JSON.parse(await readFile(new URL('../../manifest.json', import.meta.url), 'utf8'));
    assert.equal(manifest.storage, undefined, 'CRM encryption must not require managed policy setup');
  });

  it('keeps encryption keys out of content scripts and drops the legacy database', async () => {
    ...
    assert.equal(/encryptionKey|lookupKey/.test(clientSource), false,
      'content scripts must not request or handle encryption keys');
  });
});

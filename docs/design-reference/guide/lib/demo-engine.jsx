/* ───────────────────────────────────────────────────────────────
   demo-engine.jsx — the live-UI stage used across the guide.

   <LiveStage> mounts a real extension UI on a fresh mock chrome,
   frames it like the real surface, and overlays:
     • numbered callout pins + a legend  (Tour mode)
     • a synthetic cursor + step captions + player bar  (Demo mode)

   Props:
     width        device width for the frame (px)
     frameLabel   small chrome label above the UI (e.g. browser title)
     buildChrome  () => mock chrome (fresh per run)
     render       (chrome, apiRef, helpers) => ReactNode
     callouts     [{ target, n, title, text, side? }]
     steps        [{ target, caption, run?(api), hold? }]
   Exposed as window.LiveStage.
─────────────────────────────────────────────────────────────── */
(function () {
  const { motion, AnimatePresence } = window.Motion;
  const { useState, useEffect, useRef, useCallback } = React;
  const { I, Btn } = window.GB;
  ...
  function LiveStage({ width = 320, frameLabel, frameKind = 'popup', buildChrome, render, callouts = [], steps = [], note, followScroll = false }) {
    ...
    const runStep = useCallback((idx) => {
      if (idx >= steps.length) { setPlaying(false); setStepIdx(steps.length); setTimeout(() => { setRing(null); setCursor((c) => ({ ...c, visible: false })); }, 600); return; }
      const step = steps[idx];
      setStepIdx(idx);
      // measure (retry a couple frames in case layout is settling)
      let attempts = 0;
      const place = () => {
        ...
        if (followScroll) {
          const stage = stageRef.current;
          const el = stage && stage.querySelector(`[data-demo="${step.target}"]`);
          if (el) { const ab = el.getBoundingClientRect(); const ty = window.scrollY + ab.top + ab.height / 2 - window.innerHeight / 2; window.scrollTo({ top: Math.max(0, ty), behavior: 'smooth' }); }
        }
        if (r) { setCursor({ x: r.cx, y: r.cy, visible: true }); setRing(r); }
        // fire the action slightly after the cursor lands
        const actDelay = 520;
        timerRef.current = setTimeout(() => {
          try { step.run && step.run(apiRef.current, { showToast }); } catch (e) {}
          // re-measure after action (UI may have changed)
    ...
    const play = useCallback(() => {
      if (stepIdx >= steps.length - 0 && stepIdx !== -1 && !playing) { /* fallthrough */ }
      setMode('play');
      setPlaying(true);
      if (stepIdx >= steps.length || stepIdx < 0) { restart(); setTimeout(() => runStep(0), 260); }
      else runStep(stepIdx);
    }, [runStep, restart, stepIdx, steps.length, playing]);
        ...
        <div className="gb-ls-tabs">
          {callouts.length > 0 && (
            <button className={mode === 'tour' ? 'on' : ''} onClick={enterTour}>
              <I.grid size={12} /> Tour ({callouts.length})
            </button>
          )}
          {steps.length > 0 && (
          ...
          <button className={mode === 'plain' ? 'on' : ''} onClick={enterPlain}>
            <I.cube size={12} /> Try it
          </button>
          {(mode === 'play' || mode === 'plain') && (
            <button onClick={restart} title="Restart"><I.restart size={12} /> Reset</button>
          )}
        </div>
          ...
          <div className="gb-ls-framewrap">
            ...
            <div className={`gb-ls-frame ${frameKind}`} style={{ width }}>
              <div className="gb-ls-stage" ref={stageRef}>
                <div key={runKey}>{render(chrome, apiRef, helpers)}</div>
            ...
            {mode === 'play' && (
              <div className="gb-ls-player">
                ...
                <div className="gb-ls-player-btns">
                  ...
                  <Btn size="sm" variant="ghost" icon={<I.restart />} onClick={restart}>Reset</Btn>
                </div>
              </div>
            )}
            {mode === 'plain' && (

...
/* Breadcrumb trail (see contact-details): the destination trusts the
   stashed origin only when document.referrer matches it. */
...
/* Compact theme switcher — writes global gbTheme (applyTheme + saveTheme). */
/* Per-variant swatch colors (accent + deep surface), from theme.css — the
   [data-theme] rules live in the document, not our shadow, so hardcode. */
const THEME_SWATCH = {
...
/* Custom theme dropdown — trigger + themed popover with swatches. */
...
function Sidebar({ collapsed, setCollapsed }) {
  ...
  const currentLabel = 'Account · #' + (D.ids.account || '');
...
/* Inline header search — typeahead over the local CRM index; dropdown +
   Enter navigates. Does NOT open the full modal (that's the full-search
   button). "/" focuses it (shadow-safe via composedPath). */
...
/* Open our existing email viewer for the i-th email row (clicks the matching
   host tr[data-gb-ep="1"], which email-preview wired to __gbOpenEmailPreview). */
...
/* Brand letter-badges for ordered items — a colored letter mark per brand
   (T = Titleist, V = Venture, B = Bridgestone…), matched by name substring. */
const BRANDS = [

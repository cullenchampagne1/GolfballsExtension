...
/**
 * RangeSlider — two-thumb range. "Filter from X to Y".
 *
 * Props: values [low, high], min, max, step, unit,
 *   showValues (true), showRange (false), ticks, tone, disabled,
 *   onChange([low, high]).
 * Thumbs clamp to each other and cannot cross.
 */
export function RangeSlider({
  values = [25, 75], min = 0, max = 100, step = 1, unit = '',
  showValues = true, showRange = false, ticks, tone = 'brand', disabled,
  onChange, style,
}) {
  ...
  const startDrag = (e) => {
    ...
    const thumb = Math.abs(first - low) <= Math.abs(first - high) ? 'low' : 'high';
    const apply = (v) => {
      if (thumb === 'low') onChange?.([clamp(v, min, high), high]);
      else onChange?.([low, clamp(v, low, max)]);
    };
    apply(first);
    const move = (ev) => apply(valueFromX(ev.clientX));
    const up = () => {
  ...
  const pill = (label) => (
    <span style={{
      minWidth: 42, textAlign: 'center', fontSize: 11.5, fontWeight: 700,
      fontFamily: 'var(--gb-font-mono)', color: 'var(--gb-text-primary)',
      background: pillBg, border: `1px solid ${pillBd}`, borderRadius: 5,
      padding: '2px 7px', whiteSpace: 'nowrap',
    }}>{label}</span>
  );
  const thumb = (at) => (
    <div style={{
      position: 'absolute', left: `${at}%`, transform: 'translateX(-50%)',
      width: 14, height: 14, borderRadius: '50%',
      background: 'var(--gb-surface-1)', border: `2px solid ${fill}`,
      boxShadow: 'var(--gb-focus-ring)', cursor: disabled ? 'default' : 'grab', zIndex: 2,
    }} />
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, opacity: disabled ? 0.5 : 1, ...style }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        >
          ...
          <div style={{
            position: 'absolute', left: `${lowPct}%`, width: `${highPct - lowPct}%`, height: 4, borderRadius: 2,

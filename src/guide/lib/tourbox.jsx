...
/* ───────────────────────────────────────────────────────────────
   ...
   words sit next to the thing they describe. Ported from
   docs/design-reference/guide/lib/tourbox.jsx.
─────────────────────────────────────────────────────────────── */

/* A small framed surface to seat a live snippet on (popup-ish chrome).
   Auto-scales DOWN (never up) to fit its container so a fixed-width
   snippet can never overflow. Uses CSS `zoom` so the layout footprint
   shrinks too. */
export function MiniFrame({ width, label, children, pad = true }) {
...
export function TourBox({ n, title, eyebrow, live, liveLabel, children, flip, wide, stack }) {
  return (
    <div className={`tourbox ${flip ? 'flip' : ''} ${wide ? 'wide' : ''} ${stack ? 'stack' : ''}`}>
      ...
      <div className="tourbox-live">
        <div className="tourbox-livewrap">
          {liveLabel && <div className="tourbox-livelabel"><GIcon.cube size={11} /> {liveLabel}</div>}
          {live}
        </div>
      </div>
    </div>
...
export function TabbedTour({ tabs }) {

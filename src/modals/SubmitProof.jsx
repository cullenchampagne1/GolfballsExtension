...
/* Template/fake data — every row carries an account + contact list
   so the ProofCard's ConnectionStrip renders fully and the picker
   reads as live. proofLink is intentionally null while we're on
   templates; the card's copy/open buttons still animate, they just
...
export function SubmitProof({ image, orderId: orderIdProp, customerId: customerIdProp, onClosed, bindClose, onSubmitted }) {
  ...
  //   `${proofName} - ${item label}${suffix}`
  // Gift-set variants deliberately share the short label "Gift Set".
  ...
  const autoProofNames = useMemo(
    () => buildAutoProofNames(proofName, selectedItems),
  ...
  const buildProofEmail = (rows) => {
    ...
    const subject = buildProofEmailSubject({
      rush: flags.rush,
      proofNames: rows.map((r) => r.proofName),
      orderType,
      orderValue,
      orderId,
    });

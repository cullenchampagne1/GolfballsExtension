...
export function applyScales(scales) {
  ...
  lines.push('}');
  for (const c of SCALE_CATEGORIES) {
    // Popovers use the standalone `scale` CSS property (not `transform:
    // scale()`) so the rule doesn't clobber motion.div animations,
    // which write to `transform`. `scale` is a separate property that
    // composes into the same transform matrix without conflicting.
    // `zoom` would shift JS-computed coords (popovers position
    // themselves off getBoundingClientRect of an anchor) and drift off
    // the trigger.
    if (c.id === 'popovers') {
      lines.push(`[data-gb-scale="${c.id}"] { scale: var(--gb-scale-${c.id}, 1) !important; transform-origin: top left !important; }`);
    } else {

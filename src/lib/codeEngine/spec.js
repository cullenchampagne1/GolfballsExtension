/* ───────────────────────────────────────────────────────────────
   codeEngine/spec — the machine-readable workflow-code API.

   One reason to author workflows in code is that it's a natural language
   ...
   generate or edit a workflow that actually runs. Derived from the live
  ...
  return {
    ...
    bindings: {
      page: {
        ...
        'page.orders': 'array of recent CRM orders — createProposalFromOrder scans newest-first for one with a usable Duplicate Order cart; pass order explicitly only to target a specific one',
      ...
      outbound: {
        ...
        'outbound.attachProposal(result, label?)': 'append a safe customer proposal link returned by actions.createProposalFromOrder or actions.createProposal',
    ...
    rules: [
      ...
      'actions.ensureOpenOpportunity({ subject, ...createFields }) returns the first non-closed page opportunity; if none exists it creates one for page.contact. Pass its { opportunityId } into later actions.',
      'actions.createProposalFromOrder({ opportunityId, name? }) scans page.orders newest-first. It skips orders without a Duplicate Order cart or whose items need review, refreshes against current products/prices, and saves the first reusable one. Pass { order } to require one specific order.',
      'actions.createProposal({ opportunityId, items:[{ sku, quantity, price?, customLogo?, splits? }], name? }) resolves every SKU against the current catalog. Omit price for current quantity-break pricing; explicit prices remain editable overrides.',
      'Both proposal actions save the same line model used by Gift Catalog. The result appears under Current Proposals and opens with the modal’s product, decoration, variant, split, price, promo, expiration, preview, and sharing controls.',
    ...
    examples: [
      ...
      'const opportunity = await actions.ensureOpenOpportunity({ subject: "Catalog proposal" });\nconst proposal = await actions.createProposal({ opportunityId: opportunity.opportunityId, items: [{ sku: "B5338", quantity: 12 }, { sku: "M6428", quantity: 24, price: 29.95 }] });\nreturn proposal.proposalUrl;',
    ],
  };
}

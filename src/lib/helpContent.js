...
export const HELP_CONTENT = {
      ...
      "groups": [
        ...
        },
        {
          ...
          "items": [
            ...
            {
              "article": "mockup-catalog-authoring"
            },
            {
  ...
  "articles": [
    ...
    },
    {
      ...
      "body": {
        ...
        "intermediate": [
          ...
          {
            "type": "p",
            "text": "Choose Import list and open the same CSV or XLSX contact/account format accepted by CRM Search. Contact rows use their contact ID directly; account rows use the contact attached to the order nearest the same calendar date one year ago. If no order contact can be resolved, the first account contact is used as a fallback. Optional task_subject and task_description columns override the Quick Task defaults for that row. An optional sales_rep column assigns the row through the active CRM sales-rep directory: use a full name, a unique first name such as Alex, or a unique first-name plus last-initial shorthand such as AlexS. CRM options written last-name first are supported. An ambiguous or unknown rep produces a warning and that row uses the assignee chosen in Quick Task instead of being skipped. Valid recipients are selected automatically. Choose Create Tasks to set shared fallback values such as date, category, priority, and assignee, then the extension creates one task for every selected recipient. Rows without a usable contact are skipped and reported without creating orphan tasks."
          },
          {
            "type": "heading",
    ...
    },
    {
      ...
      "body": {
        "beginner": [
          {
            "type": "p",
            "text": "Open Mockup Studio from the Actions Shelf on any supported page. Start by dragging or choosing the customer's logo artwork. Once it is ready, the studio reveals the RevStack-managed product catalog and each product's available choices, so there is no account or order page requirement."
          },
          {
            "type": "heading",
          ...
          },
          {
            "type": "callout",
            "kind": "note",
            "text": "Products, option groups, exact source-image mappings, and private prompts live in the image-generation YAML. Prompt text and internal generation fields remain server-side; the client receives safe labels, option metadata, and public managed thumbnail links. The diagnostic cat test is intentionally admin-only and never appears as a product."
          }
        ],
        "intermediate": [
          {
    ...
    {
      "slug": "mockup-catalog-authoring",
      "title": "Mockup Catalog Authoring",
      "icon": "cube",
      "tiers": [
        "advanced"
      ],
      ...
      "summary": "Admin-build editor that turns a corporate-catalog custom-logo product into a managed mockup workflow — pick the variation axes, attach a reference photo per combination, and write the managed catalog without touching YAML.",
      "feature": "mockup-catalog-authoring",
      "flag": "mockupStudioEnabled",
      "covers": [
        ...
        "advanced": [
          {
            ...
            "text": "This editor is compiled only into the admin build, and the backend additionally requires an active dashboard administrator session. An installation key on its own can never reach the authoring endpoints."
          },
          {
            "type": "p",
            "text": "Open the Product Mockup Studio, then use the Add button beside the product search. The first screen lists every custom-logo product in the corporate catalog with its SKU, category, and the variation facets that product already publishes. Products that already have a mockup workflow are marked Configured and open for editing rather than creating a duplicate."
          },
          {
            "type": "p",
          ...
          },
          {
            ...
            "rows": [
              ...
              [
                "Grid cell",
                "One source with its reference image",
          ...
          },
          {
            "type": "callout",
            "kind": "info",
            "title": "The grid is meant to be sparse",
            "text": "A cell with no reference photo is simply not offered in the studio. That is normal, not an error — the shipped Venture Towel publishes 11 sources across a 2x6 grid because there is no golf-course photo of the white towel."
          },
          {
            "type": "p",
            "text": "Drop an image straight onto a cell, or select the cell and use Upload to store it in the managed reference directory. Uploads are named after the combination (studio-black.png) under the product's own folder. A per-cell prompt is optional and marked with a dot on the cell; use it when one colour or scene needs an instruction the product prompt cannot express."
          },
          {
            "type": "p",
            "text": "The product prompt at the bottom is the main instruction and must be at least 20 characters. Describe what to preserve from the reference photo and how the imprint should look. Everything above it — the axes, the grid, the placements — only decides which reference image a given selection resolves to."
          },
          {
            "type": "table",
            "headers": [
            ...
            "rows": [
              [
                "Variation axes",
                "6",
                "Additional axes cannot be enabled"
              ],
              [
                "Options per axis",
                "20",
                "Extra catalog values are dropped when the axis is built"
              ],
              [
                "References per product",
                "20",
                "The header count turns red and the draft refuses to save until you remove options"
              ],
              [
                "Product prompt",
                "20 – 16,000 characters",
                "The draft is held until the prompt is long enough"
              ]
            ]
          },
          {
            "type": "callout",
            "kind": "info",
            "title": "Saving is live, and validated first",
            "text": "Every committed change is saved automatically once the draft is valid. The backend re-validates the whole document through the studio's own catalog loader before writing, so a rejected draft leaves the live catalog untouched. The studio re-reads the managed document on every request, so a successful save is visible immediately with no restart."
          },
          {
            "type": "p",
            "text": "Each authored product also records the corporate-catalog SKU and id it came from, so a mockup workflow and its catalog product can be correlated later without guessing from the title."
          }
        ]
      },
      "sectionLabel": "Core Features / Gifting"
    },
    {
    ...
    },
    {
      ...
      "body": {
        "beginner": [
          {
            "type": "p",
            "text": "Trackers watch your work as you do it. When you create an opportunity or save a proposal in the CRM, that record is added to a table the extension keeps for you — you do not click anything, and nothing about how the CRM behaves changes. Recent orders arrive a different way: nobody's click creates them, and the CRM has no list of them to ask for, so the extension quietly re-runs the same search the CRM Search action's Scan for recent orders runs — your contacts whose last order moved since the previous look — and reads the order off each contact that comes back. That needs a CRM tab open, because the search only works while you are signed in there."
          },
          {
            "type": "table",
            "headers": [
            ...
            "rows": [
              ...
              [
                "Recent orders",
                "Found every 15 minutes by re-running your CRM search, while a CRM tab is open",
                "Not re-checked — the order already happened; a later one arrives as its own row"
              ]
            ]
          },
          {
  ...
  "searchIndex": [
    ...
    {
      "id": "article:mockup-catalog-authoring",
      "category": "Articles",
      "title": "Mockup Catalog Authoring",
      "keywords": [
        "mockup catalog",
      ...
      "description": "Admin-build editor that turns a corporate-catalog custom-logo product into a managed mockup workflow — pick the variation axes, attach a reference photo per combination, and write the managed catalog without touching YAML.",
      "article": "mockup-catalog-authoring",
      "shortcut": null,
      "flag": "mockupStudioEnabled"
    },

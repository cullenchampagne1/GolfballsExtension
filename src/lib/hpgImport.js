/* ───────────────────────────────────────────────────────────────────────────
   ...
   options become the item's Style options. Deduped by SKU on save. The optional
   `signal` ({aborted}) lets the Repo modal cancel mid-run.
   ─────────────────────────────────────────────────────────────────────────── */
...
export async function fetchHpgList({ perPage = 100, onProgress, maxPages = 500, signal } = {}) {
  const out = [];
...
export async function enrichHpg(list, { onProgress, signal } = {}) {
  const records = [];
  let done = 0;
...
export async function importHpgCatalog({ onProgress, signal } = {}) {
  const list = await fetchHpgList({ onProgress, signal });
  if (stopped(signal)) throw new Error('cancelled');
  if (!list.length) throw new Error('No customizable products found on hpgbrands');
  const records = await enrichHpg(list, { onProgress, signal });
  if (stopped(signal)) throw new Error('cancelled');
  if (!records.length) throw new Error('No HPG products parsed');
